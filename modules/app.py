...
# ---------------------------------------------------------------------------
# 外部平台兼容模型（AgentInsertRequest / AgentUpdateRequest）
# 参照 http://10.12.60.26:30541/knowledge/doc.html 智能体管理接口定义
# 冗余参数全量接收，实际只取我们需要的子集映射到 AppCreateRequest
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 模型配置 + 数据源配置接口（参照外部平台格式，数据来源为本地 DB-GPT）
# ---------------------------------------------------------------------------
@router.post("/external/model-configs")
...
@router.post("/external/datasource-configs")
...
# ---------------------------------------------------------------------------
# 兼容外部平台 insertAgent 接口的创建端点
# ---------------------------------------------------------------------------
@router.post("/insert")
async def insert_agent(req: AgentInsertRequest):
    """接收 AgentInsertRequest 格式的请求，映射为 AppCreateRequest 创建应用。
    ...
    try:
        ...
        create_req = AppCreateRequest(
            ...
            database_name=db_names[0] if db_names else "",
        ...
        body = _build_app_body(create_req)
        data = await _v1_post("/app/create", body)
        if not data.get("success"):
            err_msg = str(data.get("err_msg", data))
